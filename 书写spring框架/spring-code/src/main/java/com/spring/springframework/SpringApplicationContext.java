...
public class SpringApplicationContext {
    ...
    private void scan(Class configClass) {
        ...
        List<File> classFile = new ArrayList<>();// 存储类文件的集合
        Deque<File> dirToExplore = new ArrayDeque<>();
        if (file.isDirectory()) {
            dirToExplore.add(file);
        }
        while (!dirToExplore.isEmpty()) {
            File dir = dirToExplore.pop();
